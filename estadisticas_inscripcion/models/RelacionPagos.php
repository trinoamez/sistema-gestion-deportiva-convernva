...
class RelacionPagos {
     ...
     */
    public function getAllPagos($torneo_id = null) {
        try {
            $query = "SELECT rp.*, t.nombre as torneo_nombre, a.nombre as asociacion_nombre
                     FROM " . $this->table_name . " rp
                     LEFT JOIN torneos t ON rp.torneo_id = t.id
                     LEFT JOIN asociaciones a ON rp.asociacion_id = a.id";
            
            $params = [];
            if ($torneo_id) {
                $query .= " WHERE rp.torneo_id = ?";
                $params[] = $torneo_id;
            }
            
            $query .= " ORDER BY rp.torneo_id, rp.asociacion_id, rp.secuencia";
